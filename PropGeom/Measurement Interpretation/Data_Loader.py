...
class DataLoader:
    ...
    def load_time_signal(self, pickle_file, info_file, rpm, signal_id):
        ...
        try:
            ...
            time_axis = np.arange(len(scaled_samples)) / self.sample_rate
            ...
            return {
                ...
                "stop_time": stop_time,
                "sampling_freqcuency": self.sample_rate,
                "signal_id": signal_id,
    ...
    def data_structure(self, propeller_type, rpm, timestamp, sensor_data, pickle_file, info_file):
        ...
        for i in range(1, 11):
            ...
            try:
                ...
                if 'start_time' in signal_data and 'stop_time' in signal_data and 'start_time' not in Propeller_Measurement['sound'] and 'stop_time' not in Propeller_Measurement:
                    Propeller_Measurement["sound"]['start_time'] = signal_data['start_time']
                    Propeller_Measurement["sound"]['stop_time'] = signal_data['stop_time']
        ...
        if not Propeller_Measurement["sound"].get("start_time") or not Propeller_Measurement["sound"].get("stop_time"):
            logging.error("Measurement window (start_time, stop_time) could not be determined. Skipping processing.")
            return None  # Or raise an exception, depending on desired behavior

        # Populate sensor data
        sensor_data = sensor_data[(sensor_data['Time'] >= Propeller_Measurement["sound"]['start_time']) & (sensor_data['Time'] <= Propeller_Measurement["sound"]['stop_time'])]
    ...
    def save_to_hdf5(self, propeller_measurement, file_path):
        ...
        with h5py.File(file_path, 'w') as hdf:
            ...
            # Save sound data
            sound_group.attrs['sequence_start_time'] = propeller_measurement['sound']['start_time']
            sound_group.attrs['sequence_stop_time'] = propeller_measurement['sound']['stop_time']
        ...
        for matching_info_file in matching_info_files:
            for matching_measurement_file in matching_measurement_files:
                ...
                for rpm in range(3000, 7000, 500):
                    ...
                    # Save the data structure to an HDF5 file
